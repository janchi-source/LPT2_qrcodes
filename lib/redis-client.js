...
class RedisClient {
  ...
  spracujBuffer() {
    while (this.cakajuce.length) {
      ...
      if (p.error) {
        // Chybová odpoveď (-ERR) je odpoveď DATABÁZY, nie pád spojenia.
        // Označíme ju, aby ju command() nezamenil za výpadok siete a
        // nezahodil kvôli nej spojenie aj s cudzími rozpracovanými príkazmi.
        const e = new Error(p.error);
        e.odpovedServera = true;
        cakajuci.reject(e);
  ...
  _zahod(dovod) {
    ...
    this.buffer = Buffer.alloc(0);
    // Rozpracované príkazy patria k zahodenému spojeniu — odpoveď na ne už
    // nikdy nepríde. Keby ostali vo fronte, stali by sa dve veci naraz:
    // ich sľuby by viseli až do vypršania času funkcie, a odpovede z NOVÉHO
    // spojenia by sa priradili k nim namiesto k správnym príkazom (Redis
    // odpovedá v poradí, takže jeden zabudnutý príkaz posunie celú frontu).
    // Prakticky by potom GET vrátil napr. „OK" od AUTH a stav hry by sa
    // rozsypal — preto sa fronta musí vyprázdniť spolu so spojením.
    const e = dovod || new Error('spojenie sa zahodilo počas čakania na odpoveď');
    for (const p of this.cakajuce.splice(0)) p.reject(e);
  }
  ...
  async command(args) {
    ...
    for (let pokus = 1; pokus <= POCET_POKUSOV; pokus++) {
      try {
        ...
        // Keď odpovedala databáza (napr. „ERR unknown command"), spojenie je
        // v poriadku a opakovanie by dalo tú istú chybu — chybu teda pustíme
        // rovno von. Zahadzovať kvôli nej spojenie by navyše odstrelilo
        // súbežné príkazy, ktoré po ňom práve čakajú na odpoveď.
        if (e && e.odpovedServera) throw e;
        posledna = e;
        this._zahod(e);
