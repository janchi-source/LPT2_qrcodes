...
function defaultSettings() {
  ...
  // Štartové rozostavenie skupín — skupiny idú PROTI smeru rotácie, čiže
  // skupina g+1 je o jedno stanovište POZADU za skupinou g. Vďaka tomu príde
  // skupina g+1 v ďalšom kole presne tam, kde práve bola skupina g, takže
  // presunuté dieťa nemusí prebehnúť cez celý areál (viď harmonogram, str. 4:
  // sk.1 zasadačka, sk.2 sála, sk.3 čajovňa, …).
  const group_start_station = {};
  for (let g = 1; g <= 10; g++) {
    group_start_station[g] = stations[(stations.length + 1 - g) % stations.length].letter;
  }
  return {
    num_groups: 10,
    max_rounds: 10,
...
function defaultState(settings) {
  ...
  for (let g = 1; g <= settings.num_groups; g++) {
    groups[g] = {
      // Fallback drží to isté pravidlo ako defaultSettings — skupiny idú proti
      // smeru rotácie, aby skupina g+1 prišla tam, kde bola skupina g.
      station: settings.group_start_station[g]
        || settings.stations[modulo(settings.stations.length + 1 - g, settings.stations.length)].letter,
...
function preflight() {
  ...
  // Koľko kôl trvá JEDEN posun o skupinu. Pri prahu b ostane dieťa b-1 kôl ako
  // bait a až potom sa posunie, čiže jeden posun = b kôl. Prah 0 aj 1 znamenajú
  // posun hneď, teda 1 kolo.
  const b = settings.bait || {};
  const kolNaPresun = b.mode === 'random'
    ? Math.max(1, parseInt(b.random_max, 10) || 0)
    : Math.max(1, parseInt(b.delay_rounds, 10) || 1);

  // Vzdialenosť = počet posunov +1, ktoré dieťa potrebuje domov.
  // Keď sú deti už rozdelené, počítame zo skutočnosti; inak z najhoršieho
  // možného prípadu, ktorý rozdelenie môže vyrobiť (g-1).
  ...
  if (!poistkaPlati && potrebnychKol > settings.max_rounds) {
    chyby.push(`Najvzdialenejšie dieťa potrebuje ${potrebnychKol} kôl `
      + `(${najvacsiaVzdialenost} posunov × ${kolNaPresun} ${kolNaPresun === 1 ? 'kolo' : 'kôl'} na posun), `
      + `ale hra má ${settings.max_rounds}. Časť detí by sa domov nedostala. `
      + (kolNaPresun > 1
        ? 'Nastav bait prah na 1 (presun hneď pri prvom skene).'
        : `Zvýš počet kôl aspoň na ${potrebnychKol}.`));
  }
