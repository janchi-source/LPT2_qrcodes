...
// Skúšobné pripojenie k databáze pre /api/db-test. Vracia zrozumiteľnú
// diagnostiku namiesto surovej chyby — bez prístupových údajov.
async function ping() {
  if (!useKV) {
  ...
  try {
    const odpoved = await redis(['PING']);
    return { ok: true, mode: useREST ? 'rest' : 'tcp', odpoved: String(odpoved), ms: Date.now() - zaciatok };
  } catch (e) {
    return { ok: false, mode: useREST ? 'rest' : 'tcp', ms: Date.now() - zaciatok, chyba: String((e && e.message) || e) };
  }
}
