// Úložisko stavu hry. Backend sa vyberie sám podľa nastavených premenných:
//
//   'supabase'        — Postgres cez HTTPS (lib/db-supabase.js). Odporúčaný
//                       pre hosting: atomický zápis robí priamo databáza
//                       (UPDATE ... WHERE verzia = N), bez Lua skriptov aj bez
//                       držaného spojenia.
//   'rest' / 'tcp'    — Redis (Vercel KV / Upstash). Ostáva podporovaný, nech
//                       staršie nasadenie funguje ďalej.
...
//
// Riešenie: v databázovom režime sa na začiatku požiadavky načíta CELÝ stav do
// pamäte, herná logika beží nad týmto snapshotom synchrónne ako doteraz, a na
// konci sa zapíše jedným atomickým zápisom, ktorý prejde len ak sa medzitým
// stav nezmenil. Pri konflikte sa požiadavka zopakuje nad čerstvými dátami.
const fs = require('fs');
...
// Redis môže hosting dať dvoma spôsobmi:
//   REST  — KV_REST_API_URL + KV_REST_API_TOKEN (Vercel KV / Upstash REST)
//   TCP   — REDIS_URL, čiže rediss://user:heslo@host:port
const KV_URL = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL || '';
const KV_TOKEN = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN || '';
const REDIS_URL = process.env.REDIS_URL || process.env.REDIS_TLS_URL || '';

// Supabase má prednosť: keď je nastavený, je to vedomá voľba a nechceme, aby
// appka ticho písala do starého Redisu, ktorý v projekte ostal priradený.
const useSupabase = supabase.dostupne;
const useREST = !useSupabase && !!(KV_URL && KV_TOKEN);
const useTCP = !useSupabase && !useREST && !!REDIS_URL;
const useKV = useSupabase || useREST || useTCP;
...
const redisBackend = {
  nazov: useREST ? 'rest' : 'tcp',

  async nacitaj() {
    const [doc, ver] = await Promise.all([redis(['GET', DOC_KEY]), redis(['GET', VER_KEY])]);
    return { data: doc ? JSON.parse(doc) : {}, version: ver == null ? '0' : String(ver) };
  },

  async zapis(data, version) {
    const zapisane = await redis([
      'EVAL', CAS_SCRIPT, '2', DOC_KEY, VER_KEY, JSON.stringify(data), version,
    ]);
    return Number(zapisane) === 1;
  },

  // Neoveruje len spojenie, ale celý cyklus, ktorý appka počas hry potrebuje —
  // vrátane Lua skriptu (EVAL), na ktorom stojí atomický zápis. Niektorí
  // poskytovatelia EVAL zakazujú a prejavilo by sa to až pri prvom skene.
  async skuska() {
...
// Ktorý backend sa naozaj používa. null = súborový režim.
const backend = useSupabase ? supabase : (useKV ? redisBackend : null);
...
//
// Súborový režim: číta a zapisuje priamo na disk. Node je jednovláknový a `fn`
// je celá synchrónna, takže čítaj-uprav-zapíš dobehne bez prerušenia.
//
// Databázový režim: načíta celý stav, spustí `fn` nad ním a zapíše ho jedným
...
async function runTransaction(fn) {
  if (!backend) return { ok: true, result: fn(), pokusy: 1 };
  ...
  for (let pokus = 1; pokus <= MAX_POKUSOV; pokus++) {
    ...
    const { data, version } = await backend.nacitaj();
    const ctx = { data, version, dirty: false };
    ...
    if (await backend.zapis(ctx.data, ctx.version)) return { ok: true, result, pokusy: pokus };
...
// Skúška databázy pre /api/db-test. Musí fungovať aj keď databáza nejde —
...
async function ping() {
  if (!backend) {
    return {
      ok: false,
      mode: 'file',
      chyba: 'Nie je nastavená žiadna databáza (chýba SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY, '
        + 'ani REDIS_URL či REST údaje).',
    };
  }
  return backend.skuska();
}

function load(name, fallback) {
  const ctx = als.getStore();
  if (ctx) return Object.prototype.hasOwnProperty.call(ctx.data, name) ? ctx.data[name] : fallback;
  if (backend) throw new Error('databázový režim: čítanie stavu mimo runTransaction()');
...
function save(name, obj) {
  ...
  if (backend) throw new Error('databázový režim: zápis stavu mimo runTransaction()');
  writeFile(name, obj);
}
...
module.exports = {
  ...
  ping,
  useKV,
  // 'supabase' | 'rest' | 'tcp' | 'file' — vypisuje sa v /api/server-info
  mode: useSupabase ? 'supabase' : (useREST ? 'rest' : (useTCP ? 'tcp' : 'file')),
  DATA_DIR,
